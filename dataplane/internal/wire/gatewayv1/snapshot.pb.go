// Code generated by protoc-gen-go. DO NOT EDIT.
// source: gateway/v1/snapshot.proto

// The snapshot wire format: the contract between the Python control plane that
// compiles configuration and the Go data plane that serves requests from it.
//
// This is deliberately NOT the same as the Go domain types in internal/core.
// Domain types are shaped for the request path and are refactored freely; wire
// types are append-only and backward-compatible forever. Collapsing the two
// would make every internal rename a wire-compatibility question.
//
// Compatibility rules for anyone editing this file:
//   - Never reuse or renumber a field tag. Use `reserved` when removing one.
//   - New fields are always optional in effect: an older worker must be able to
//     read a newer layer and ignore what it does not understand, because during
//     a rollout both versions are live at once.
//   - Enums always keep an UNSPECIFIED zero value, so a value added later
//     arrives as "unspecified" rather than being silently read as the first
//     real variant.

package gatewayv1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type TrustTier int32

const (
	TrustTier_TRUST_TIER_UNSPECIFIED   TrustTier = 0
	TrustTier_TRUST_TIER_EXTERNAL      TrustTier = 1
	TrustTier_TRUST_TIER_PRIVATE_CLOUD TrustTier = 2
	TrustTier_TRUST_TIER_INTERNAL      TrustTier = 3
)

// Enum value maps for TrustTier.
var (
	TrustTier_name = map[int32]string{
		0: "TRUST_TIER_UNSPECIFIED",
		1: "TRUST_TIER_EXTERNAL",
		2: "TRUST_TIER_PRIVATE_CLOUD",
		3: "TRUST_TIER_INTERNAL",
	}
	TrustTier_value = map[string]int32{
		"TRUST_TIER_UNSPECIFIED":   0,
		"TRUST_TIER_EXTERNAL":      1,
		"TRUST_TIER_PRIVATE_CLOUD": 2,
		"TRUST_TIER_INTERNAL":      3,
	}
)

func (x TrustTier) Enum() *TrustTier {
	p := new(TrustTier)
	*p = x
	return p
}

func (x TrustTier) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (TrustTier) Descriptor() protoreflect.EnumDescriptor {
	return file_gateway_v1_snapshot_proto_enumTypes[0].Descriptor()
}

func (TrustTier) Type() protoreflect.EnumType {
	return &file_gateway_v1_snapshot_proto_enumTypes[0]
}

func (x TrustTier) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use TrustTier.Descriptor instead.
func (TrustTier) EnumDescriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{0}
}

type BudgetScope int32

const (
	BudgetScope_BUDGET_SCOPE_UNSPECIFIED BudgetScope = 0
	BudgetScope_BUDGET_SCOPE_KEY         BudgetScope = 1
	BudgetScope_BUDGET_SCOPE_APP         BudgetScope = 2
	BudgetScope_BUDGET_SCOPE_USER        BudgetScope = 3
	BudgetScope_BUDGET_SCOPE_TEAM        BudgetScope = 4
	BudgetScope_BUDGET_SCOPE_ORG         BudgetScope = 5
	BudgetScope_BUDGET_SCOPE_MODEL       BudgetScope = 6
	BudgetScope_BUDGET_SCOPE_TRAINING    BudgetScope = 7
)

// Enum value maps for BudgetScope.
var (
	BudgetScope_name = map[int32]string{
		0: "BUDGET_SCOPE_UNSPECIFIED",
		1: "BUDGET_SCOPE_KEY",
		2: "BUDGET_SCOPE_APP",
		3: "BUDGET_SCOPE_USER",
		4: "BUDGET_SCOPE_TEAM",
		5: "BUDGET_SCOPE_ORG",
		6: "BUDGET_SCOPE_MODEL",
		7: "BUDGET_SCOPE_TRAINING",
	}
	BudgetScope_value = map[string]int32{
		"BUDGET_SCOPE_UNSPECIFIED": 0,
		"BUDGET_SCOPE_KEY":         1,
		"BUDGET_SCOPE_APP":         2,
		"BUDGET_SCOPE_USER":        3,
		"BUDGET_SCOPE_TEAM":        4,
		"BUDGET_SCOPE_ORG":         5,
		"BUDGET_SCOPE_MODEL":       6,
		"BUDGET_SCOPE_TRAINING":    7,
	}
)

func (x BudgetScope) Enum() *BudgetScope {
	p := new(BudgetScope)
	*p = x
	return p
}

func (x BudgetScope) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (BudgetScope) Descriptor() protoreflect.EnumDescriptor {
	return file_gateway_v1_snapshot_proto_enumTypes[1].Descriptor()
}

func (BudgetScope) Type() protoreflect.EnumType {
	return &file_gateway_v1_snapshot_proto_enumTypes[1]
}

func (x BudgetScope) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use BudgetScope.Descriptor instead.
func (BudgetScope) EnumDescriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{1}
}

type Port int32

const (
	Port_PORT_UNSPECIFIED Port = 0
	Port_PORT_PROVIDER    Port = 1
	Port_PORT_GUARDRAIL   Port = 2
	Port_PORT_STORE       Port = 3
	Port_PORT_TELEMETRY   Port = 4
)

// Enum value maps for Port.
var (
	Port_name = map[int32]string{
		0: "PORT_UNSPECIFIED",
		1: "PORT_PROVIDER",
		2: "PORT_GUARDRAIL",
		3: "PORT_STORE",
		4: "PORT_TELEMETRY",
	}
	Port_value = map[string]int32{
		"PORT_UNSPECIFIED": 0,
		"PORT_PROVIDER":    1,
		"PORT_GUARDRAIL":   2,
		"PORT_STORE":       3,
		"PORT_TELEMETRY":   4,
	}
)

func (x Port) Enum() *Port {
	p := new(Port)
	*p = x
	return p
}

func (x Port) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (Port) Descriptor() protoreflect.EnumDescriptor {
	return file_gateway_v1_snapshot_proto_enumTypes[2].Descriptor()
}

func (Port) Type() protoreflect.EnumType {
	return &file_gateway_v1_snapshot_proto_enumTypes[2]
}

func (x Port) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use Port.Descriptor instead.
func (Port) EnumDescriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{2}
}

// FailureMode is what to do when a guardrail errors or exceeds its budget.
type FailureMode int32

const (
	FailureMode_FAILURE_MODE_UNSPECIFIED FailureMode = 0
	// Allow the request. For detection that is useful but not authoritative —
	// prompt-injection heuristics are the example the design names, because
	// shipping them as a blocking control buys an outage risk in exchange for
	// security theatre.
	FailureMode_FAILURE_MODE_OPEN FailureMode = 1
	// Refuse the request. For controls where the failure is not recoverable: a
	// leaked credential cannot be un-leaked.
	FailureMode_FAILURE_MODE_CLOSED FailureMode = 2
)

// Enum value maps for FailureMode.
var (
	FailureMode_name = map[int32]string{
		0: "FAILURE_MODE_UNSPECIFIED",
		1: "FAILURE_MODE_OPEN",
		2: "FAILURE_MODE_CLOSED",
	}
	FailureMode_value = map[string]int32{
		"FAILURE_MODE_UNSPECIFIED": 0,
		"FAILURE_MODE_OPEN":        1,
		"FAILURE_MODE_CLOSED":      2,
	}
)

func (x FailureMode) Enum() *FailureMode {
	p := new(FailureMode)
	*p = x
	return p
}

func (x FailureMode) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (FailureMode) Descriptor() protoreflect.EnumDescriptor {
	return file_gateway_v1_snapshot_proto_enumTypes[3].Descriptor()
}

func (FailureMode) Type() protoreflect.EnumType {
	return &file_gateway_v1_snapshot_proto_enumTypes[3]
}

func (x FailureMode) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use FailureMode.Descriptor instead.
func (FailureMode) EnumDescriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{3}
}

// PolicyEffect is what a matching rule does.
type PolicyEffect int32

const (
	PolicyEffect_POLICY_EFFECT_UNSPECIFIED PolicyEffect = 0
	PolicyEffect_POLICY_EFFECT_ALLOW       PolicyEffect = 1
	PolicyEffect_POLICY_EFFECT_DENY        PolicyEffect = 2
)

// Enum value maps for PolicyEffect.
var (
	PolicyEffect_name = map[int32]string{
		0: "POLICY_EFFECT_UNSPECIFIED",
		1: "POLICY_EFFECT_ALLOW",
		2: "POLICY_EFFECT_DENY",
	}
	PolicyEffect_value = map[string]int32{
		"POLICY_EFFECT_UNSPECIFIED": 0,
		"POLICY_EFFECT_ALLOW":       1,
		"POLICY_EFFECT_DENY":        2,
	}
)

func (x PolicyEffect) Enum() *PolicyEffect {
	p := new(PolicyEffect)
	*p = x
	return p
}

func (x PolicyEffect) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (PolicyEffect) Descriptor() protoreflect.EnumDescriptor {
	return file_gateway_v1_snapshot_proto_enumTypes[4].Descriptor()
}

func (PolicyEffect) Type() protoreflect.EnumType {
	return &file_gateway_v1_snapshot_proto_enumTypes[4]
}

func (x PolicyEffect) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use PolicyEffect.Descriptor instead.
func (PolicyEffect) EnumDescriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{4}
}

// LayerVersion identifies one build of one layer. Number orders versions;
// digest content-addresses the bytes so a worker can skip a re-fetch and two
// workers claiming the same version can be proven to hold the same layer.
type LayerVersion struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Number        uint64                 `protobuf:"varint,1,opt,name=number,proto3" json:"number,omitempty"`
	Digest        string                 `protobuf:"bytes,2,opt,name=digest,proto3" json:"digest,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LayerVersion) Reset() {
	*x = LayerVersion{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LayerVersion) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LayerVersion) ProtoMessage() {}

func (x *LayerVersion) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LayerVersion.ProtoReflect.Descriptor instead.
func (*LayerVersion) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{0}
}

func (x *LayerVersion) GetNumber() uint64 {
	if x != nil {
		return x.Number
	}
	return 0
}

func (x *LayerVersion) GetDigest() string {
	if x != nil {
		return x.Digest
	}
	return ""
}

// RoutingKey is (base model, adapter). Adapter is empty for a plain base model.
// Multi-LoRA serving means one base deployment serves many adapters, so this is
// the routing table's key from the first release rather than a bare model name.
type RoutingKey struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	BaseModel     string                 `protobuf:"bytes,1,opt,name=base_model,json=baseModel,proto3" json:"base_model,omitempty"`
	AdapterId     string                 `protobuf:"bytes,2,opt,name=adapter_id,json=adapterId,proto3" json:"adapter_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RoutingKey) Reset() {
	*x = RoutingKey{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RoutingKey) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RoutingKey) ProtoMessage() {}

func (x *RoutingKey) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RoutingKey.ProtoReflect.Descriptor instead.
func (*RoutingKey) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{1}
}

func (x *RoutingKey) GetBaseModel() string {
	if x != nil {
		return x.BaseModel
	}
	return ""
}

func (x *RoutingKey) GetAdapterId() string {
	if x != nil {
		return x.AdapterId
	}
	return ""
}

// Cost is what a deployment charges, per thousand tokens of each class.
//
// Millionths of a US dollar, integer, never floating point: per-token prices
// are small enough that float rounding reaches the invoice.
//
// The classes exist because providers do not bill all input tokens the same.
// Cached input is an order of magnitude cheaper, and writing to a cache is
// dearer than a plain read. Recording only a single input rate means a request
// that was 90% cache reads is billed as though it were not — and the provider
// told us the truth exactly once, in a response we did not keep.
type Cost struct {
	state                protoimpl.MessageState `protogen:"open.v1"`
	InputPer_1KMicroUsd  int64                  `protobuf:"varint,1,opt,name=input_per_1k_micro_usd,json=inputPer1kMicroUsd,proto3" json:"input_per_1k_micro_usd,omitempty"`
	OutputPer_1KMicroUsd int64                  `protobuf:"varint,2,opt,name=output_per_1k_micro_usd,json=outputPer1kMicroUsd,proto3" json:"output_per_1k_micro_usd,omitempty"`
	// Zero means unconfigured, and the caller falls back to the standard input
	// rate. Falling back over-bills slightly rather than under-billing, which is
	// the safe direction for a number a customer disputes.
	CachedInputPer_1KMicroUsd int64 `protobuf:"varint,3,opt,name=cached_input_per_1k_micro_usd,json=cachedInputPer1kMicroUsd,proto3" json:"cached_input_per_1k_micro_usd,omitempty"`
	CacheWritePer_1KMicroUsd  int64 `protobuf:"varint,4,opt,name=cache_write_per_1k_micro_usd,json=cacheWritePer1kMicroUsd,proto3" json:"cache_write_per_1k_micro_usd,omitempty"`
	unknownFields             protoimpl.UnknownFields
	sizeCache                 protoimpl.SizeCache
}

func (x *Cost) Reset() {
	*x = Cost{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Cost) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Cost) ProtoMessage() {}

func (x *Cost) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Cost.ProtoReflect.Descriptor instead.
func (*Cost) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{2}
}

func (x *Cost) GetInputPer_1KMicroUsd() int64 {
	if x != nil {
		return x.InputPer_1KMicroUsd
	}
	return 0
}

func (x *Cost) GetOutputPer_1KMicroUsd() int64 {
	if x != nil {
		return x.OutputPer_1KMicroUsd
	}
	return 0
}

func (x *Cost) GetCachedInputPer_1KMicroUsd() int64 {
	if x != nil {
		return x.CachedInputPer_1KMicroUsd
	}
	return 0
}

func (x *Cost) GetCacheWritePer_1KMicroUsd() int64 {
	if x != nil {
		return x.CacheWritePer_1KMicroUsd
	}
	return 0
}

type Deployment struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	Id        string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Key       *RoutingKey            `protobuf:"bytes,2,opt,name=key,proto3" json:"key,omitempty"`
	Provider  string                 `protobuf:"bytes,3,opt,name=provider,proto3" json:"provider,omitempty"`
	Endpoint  string                 `protobuf:"bytes,4,opt,name=endpoint,proto3" json:"endpoint,omitempty"`
	Region    string                 `protobuf:"bytes,5,opt,name=region,proto3" json:"region,omitempty"`
	TrustTier TrustTier              `protobuf:"varint,6,opt,name=trust_tier,json=trustTier,proto3,enum=gateway.v1.TrustTier" json:"trust_tier,omitempty"`
	// A reference to a secret, never the secret. Provider credentials must not
	// enter a snapshot: snapshots are replicated to every worker, cached and
	// versioned. Workers resolve the reference separately and cache with a TTL.
	CredentialRef string `protobuf:"bytes,7,opt,name=credential_ref,json=credentialRef,proto3" json:"credential_ref,omitempty"`
	// Share of traffic, 0-100. Zero means registered but not serving — the state
	// a fine-tuned adapter sits in while it takes shadow traffic.
	Weight        uint32   `protobuf:"varint,8,opt,name=weight,proto3" json:"weight,omitempty"`
	Cost          *Cost    `protobuf:"bytes,9,opt,name=cost,proto3" json:"cost,omitempty"`
	Capabilities  []string `protobuf:"bytes,10,rep,name=capabilities,proto3" json:"capabilities,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Deployment) Reset() {
	*x = Deployment{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Deployment) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Deployment) ProtoMessage() {}

func (x *Deployment) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Deployment.ProtoReflect.Descriptor instead.
func (*Deployment) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{3}
}

func (x *Deployment) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Deployment) GetKey() *RoutingKey {
	if x != nil {
		return x.Key
	}
	return nil
}

func (x *Deployment) GetProvider() string {
	if x != nil {
		return x.Provider
	}
	return ""
}

func (x *Deployment) GetEndpoint() string {
	if x != nil {
		return x.Endpoint
	}
	return ""
}

func (x *Deployment) GetRegion() string {
	if x != nil {
		return x.Region
	}
	return ""
}

func (x *Deployment) GetTrustTier() TrustTier {
	if x != nil {
		return x.TrustTier
	}
	return TrustTier_TRUST_TIER_UNSPECIFIED
}

func (x *Deployment) GetCredentialRef() string {
	if x != nil {
		return x.CredentialRef
	}
	return ""
}

func (x *Deployment) GetWeight() uint32 {
	if x != nil {
		return x.Weight
	}
	return 0
}

func (x *Deployment) GetCost() *Cost {
	if x != nil {
		return x.Cost
	}
	return nil
}

func (x *Deployment) GetCapabilities() []string {
	if x != nil {
		return x.Capabilities
	}
	return nil
}

type ModelAlias struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Name          string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	Targets       []*RoutingKey          `protobuf:"bytes,2,rep,name=targets,proto3" json:"targets,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ModelAlias) Reset() {
	*x = ModelAlias{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ModelAlias) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ModelAlias) ProtoMessage() {}

func (x *ModelAlias) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ModelAlias.ProtoReflect.Descriptor instead.
func (*ModelAlias) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{4}
}

func (x *ModelAlias) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ModelAlias) GetTargets() []*RoutingKey {
	if x != nil {
		return x.Targets
	}
	return nil
}

// PolicyRule is one row of the compiled decision table.
//
// Every condition is a set the attribute must be in, and an empty set matches
// anything. All present conditions must hold, so a rule reads as a conjunction
// and can be understood without reference to any other rule.
//
// This is deliberately not a language: no negation, no recursion, no joins, no
// data documents. See docs/adr/0006-compiled-policy-not-a-policy-language.md.
type PolicyRule struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Id     string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Effect PolicyEffect           `protobuf:"varint,2,opt,name=effect,proto3,enum=gateway.v1.PolicyEffect" json:"effect,omitempty"`
	// Conditions. Empty means "any".
	Models    []string `protobuf:"bytes,3,rep,name=models,proto3" json:"models,omitempty"`
	Endpoints []string `protobuf:"bytes,4,rep,name=endpoints,proto3" json:"endpoints,omitempty"`
	Roles     []string `protobuf:"bytes,5,rep,name=roles,proto3" json:"roles,omitempty"`
	Regions   []string `protobuf:"bytes,6,rep,name=regions,proto3" json:"regions,omitempty"`
	// source_cidrs are CIDR blocks the caller's address must fall inside.
	SourceCidrs []string `protobuf:"bytes,7,rep,name=source_cidrs,json=sourceCidrs,proto3" json:"source_cidrs,omitempty"`
	// max_payload_bytes is an upper bound; zero means no bound.
	MaxPayloadBytes uint64 `protobuf:"varint,8,opt,name=max_payload_bytes,json=maxPayloadBytes,proto3" json:"max_payload_bytes,omitempty"`
	// Effects applied when an allow rule matches. Stamping here is what lets
	// policy decide sensitivity, which the router then turns into a destination
	// constraint — the ordering the design requires.
	DataClass    string    `protobuf:"bytes,9,opt,name=data_class,json=dataClass,proto3" json:"data_class,omitempty"`
	MinTrustTier TrustTier `protobuf:"varint,10,opt,name=min_trust_tier,json=minTrustTier,proto3,enum=gateway.v1.TrustTier" json:"min_trust_tier,omitempty"`
	// reason is returned to the caller on a denial. It is written by an operator
	// and is expected to be safe to disclose.
	Reason        string `protobuf:"bytes,11,opt,name=reason,proto3" json:"reason,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PolicyRule) Reset() {
	*x = PolicyRule{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PolicyRule) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PolicyRule) ProtoMessage() {}

func (x *PolicyRule) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PolicyRule.ProtoReflect.Descriptor instead.
func (*PolicyRule) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{5}
}

func (x *PolicyRule) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *PolicyRule) GetEffect() PolicyEffect {
	if x != nil {
		return x.Effect
	}
	return PolicyEffect_POLICY_EFFECT_UNSPECIFIED
}

func (x *PolicyRule) GetModels() []string {
	if x != nil {
		return x.Models
	}
	return nil
}

func (x *PolicyRule) GetEndpoints() []string {
	if x != nil {
		return x.Endpoints
	}
	return nil
}

func (x *PolicyRule) GetRoles() []string {
	if x != nil {
		return x.Roles
	}
	return nil
}

func (x *PolicyRule) GetRegions() []string {
	if x != nil {
		return x.Regions
	}
	return nil
}

func (x *PolicyRule) GetSourceCidrs() []string {
	if x != nil {
		return x.SourceCidrs
	}
	return nil
}

func (x *PolicyRule) GetMaxPayloadBytes() uint64 {
	if x != nil {
		return x.MaxPayloadBytes
	}
	return 0
}

func (x *PolicyRule) GetDataClass() string {
	if x != nil {
		return x.DataClass
	}
	return ""
}

func (x *PolicyRule) GetMinTrustTier() TrustTier {
	if x != nil {
		return x.MinTrustTier
	}
	return TrustTier_TRUST_TIER_UNSPECIFIED
}

func (x *PolicyRule) GetReason() string {
	if x != nil {
		return x.Reason
	}
	return ""
}

// PolicyBundle is the compiled decision table.
//
// Compiled in the control plane, never interpreted per request: a gateway that
// parses policy on every request has put a language runtime in its hot path,
// which is what the snapshot model exists to avoid.
type PolicyBundle struct {
	state   protoimpl.MessageState `protogen:"open.v1"`
	Id      string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Version uint64                 `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	// Rules in evaluation order. First match wins, which makes an ordering
	// an operator can read the whole of the conflict-resolution semantics.
	Rules []*PolicyRule `protobuf:"bytes,3,rep,name=rules,proto3" json:"rules,omitempty"`
	// default_effect applies when no rule matches. Allow, because policy is one
	// control among several and a bundle that denied by default would make
	// adding the feature an outage.
	DefaultEffect PolicyEffect `protobuf:"varint,4,opt,name=default_effect,json=defaultEffect,proto3,enum=gateway.v1.PolicyEffect" json:"default_effect,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PolicyBundle) Reset() {
	*x = PolicyBundle{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PolicyBundle) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PolicyBundle) ProtoMessage() {}

func (x *PolicyBundle) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PolicyBundle.ProtoReflect.Descriptor instead.
func (*PolicyBundle) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{6}
}

func (x *PolicyBundle) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *PolicyBundle) GetVersion() uint64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *PolicyBundle) GetRules() []*PolicyRule {
	if x != nil {
		return x.Rules
	}
	return nil
}

func (x *PolicyBundle) GetDefaultEffect() PolicyEffect {
	if x != nil {
		return x.DefaultEffect
	}
	return PolicyEffect_POLICY_EFFECT_UNSPECIFIED
}

// GuardrailBinding is one guardrail and the budget it is admitted under.
//
// A list rather than one binding per port, because a tenant genuinely runs
// several: a secret scan that blocks, an injection heuristic that only alerts,
// a content policy. Modelling it as one-per-port would have forced them into
// one component and made their budgets indistinguishable.
type GuardrailBinding struct {
	state     protoimpl.MessageState `protogen:"open.v1"`
	Component string                 `protobuf:"bytes,1,opt,name=component,proto3" json:"component,omitempty"`
	Version   string                 `protobuf:"bytes,2,opt,name=version,proto3" json:"version,omitempty"`
	ConfigRef string                 `protobuf:"bytes,3,opt,name=config_ref,json=configRef,proto3" json:"config_ref,omitempty"`
	// timeout_ms is enforced by the caller rather than trusted to the
	// implementation. A guardrail that hangs must not be able to hang the
	// request, and asking it to police itself assumes the thing that failed.
	TimeoutMs   uint32      `protobuf:"varint,4,opt,name=timeout_ms,json=timeoutMs,proto3" json:"timeout_ms,omitempty"`
	FailureMode FailureMode `protobuf:"varint,5,opt,name=failure_mode,json=failureMode,proto3,enum=gateway.v1.FailureMode" json:"failure_mode,omitempty"`
	// blocking guardrails run inline and can refuse or rewrite. Non-blocking
	// ones run off the request path on a copy and can only alert — which is the
	// honest shape for anything whose accuracy does not justify refusing real
	// traffic.
	Blocking bool `protobuf:"varint,6,opt,name=blocking,proto3" json:"blocking,omitempty"`
	// phases the guardrail inspects. Empty means the request leg only.
	Phases        []string `protobuf:"bytes,7,rep,name=phases,proto3" json:"phases,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GuardrailBinding) Reset() {
	*x = GuardrailBinding{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GuardrailBinding) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GuardrailBinding) ProtoMessage() {}

func (x *GuardrailBinding) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GuardrailBinding.ProtoReflect.Descriptor instead.
func (*GuardrailBinding) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{7}
}

func (x *GuardrailBinding) GetComponent() string {
	if x != nil {
		return x.Component
	}
	return ""
}

func (x *GuardrailBinding) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

func (x *GuardrailBinding) GetConfigRef() string {
	if x != nil {
		return x.ConfigRef
	}
	return ""
}

func (x *GuardrailBinding) GetTimeoutMs() uint32 {
	if x != nil {
		return x.TimeoutMs
	}
	return 0
}

func (x *GuardrailBinding) GetFailureMode() FailureMode {
	if x != nil {
		return x.FailureMode
	}
	return FailureMode_FAILURE_MODE_UNSPECIFIED
}

func (x *GuardrailBinding) GetBlocking() bool {
	if x != nil {
		return x.Blocking
	}
	return false
}

func (x *GuardrailBinding) GetPhases() []string {
	if x != nil {
		return x.Phases
	}
	return nil
}

type PluginBinding struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Port          Port                   `protobuf:"varint,1,opt,name=port,proto3,enum=gateway.v1.Port" json:"port,omitempty"`
	Component     string                 `protobuf:"bytes,2,opt,name=component,proto3" json:"component,omitempty"`
	Version       string                 `protobuf:"bytes,3,opt,name=version,proto3" json:"version,omitempty"`
	ConfigRef     string                 `protobuf:"bytes,4,opt,name=config_ref,json=configRef,proto3" json:"config_ref,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PluginBinding) Reset() {
	*x = PluginBinding{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PluginBinding) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PluginBinding) ProtoMessage() {}

func (x *PluginBinding) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PluginBinding.ProtoReflect.Descriptor instead.
func (*PluginBinding) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{8}
}

func (x *PluginBinding) GetPort() Port {
	if x != nil {
		return x.Port
	}
	return Port_PORT_UNSPECIFIED
}

func (x *PluginBinding) GetComponent() string {
	if x != nil {
		return x.Component
	}
	return ""
}

func (x *PluginBinding) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

func (x *PluginBinding) GetConfigRef() string {
	if x != nil {
		return x.ConfigRef
	}
	return ""
}

type BudgetState struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Scope         BudgetScope            `protobuf:"varint,2,opt,name=scope,proto3,enum=gateway.v1.BudgetScope" json:"scope,omitempty"`
	LimitMicroUsd int64                  `protobuf:"varint,3,opt,name=limit_micro_usd,json=limitMicroUsd,proto3" json:"limit_micro_usd,omitempty"`
	SpentMicroUsd int64                  `protobuf:"varint,4,opt,name=spent_micro_usd,json=spentMicroUsd,proto3" json:"spent_micro_usd,omitempty"`
	Hard          bool                   `protobuf:"varint,5,opt,name=hard,proto3" json:"hard,omitempty"`
	// Basis points of the limit held back so requests already streaming when the
	// budget tips can finish without overshooting. 500 = 5%.
	HeadroomBasisPoints uint32 `protobuf:"varint,6,opt,name=headroom_basis_points,json=headroomBasisPoints,proto3" json:"headroom_basis_points,omitempty"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *BudgetState) Reset() {
	*x = BudgetState{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BudgetState) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BudgetState) ProtoMessage() {}

func (x *BudgetState) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BudgetState.ProtoReflect.Descriptor instead.
func (*BudgetState) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{9}
}

func (x *BudgetState) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *BudgetState) GetScope() BudgetScope {
	if x != nil {
		return x.Scope
	}
	return BudgetScope_BUDGET_SCOPE_UNSPECIFIED
}

func (x *BudgetState) GetLimitMicroUsd() int64 {
	if x != nil {
		return x.LimitMicroUsd
	}
	return 0
}

func (x *BudgetState) GetSpentMicroUsd() int64 {
	if x != nil {
		return x.SpentMicroUsd
	}
	return 0
}

func (x *BudgetState) GetHard() bool {
	if x != nil {
		return x.Hard
	}
	return false
}

func (x *BudgetState) GetHeadroomBasisPoints() uint32 {
	if x != nil {
		return x.HeadroomBasisPoints
	}
	return 0
}

type BudgetRef struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Scope         BudgetScope            `protobuf:"varint,2,opt,name=scope,proto3,enum=gateway.v1.BudgetScope" json:"scope,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BudgetRef) Reset() {
	*x = BudgetRef{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BudgetRef) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BudgetRef) ProtoMessage() {}

func (x *BudgetRef) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BudgetRef.ProtoReflect.Descriptor instead.
func (*BudgetRef) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{10}
}

func (x *BudgetRef) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *BudgetRef) GetScope() BudgetScope {
	if x != nil {
		return x.Scope
	}
	return BudgetScope_BUDGET_SCOPE_UNSPECIFIED
}

// RateLimit is what a principal may consume per window.
//
// Zero means unlimited for that dimension, which is the only sane default: a
// principal created before a limit existed must not suddenly be capped at zero.
type RateLimit struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Requests admitted per minute.
	RequestsPerMinute uint32 `protobuf:"varint,1,opt,name=requests_per_minute,json=requestsPerMinute,proto3" json:"requests_per_minute,omitempty"`
	// Tokens per minute, counted from reported usage, so it necessarily lags by
	// one request. Approximate by construction and documented as such.
	TokensPerMinute uint32 `protobuf:"varint,2,opt,name=tokens_per_minute,json=tokensPerMinute,proto3" json:"tokens_per_minute,omitempty"`
	// Requests in flight at once. Enforced per worker, so the fleet-wide figure
	// is this multiplied by worker count — stated rather than pretended away.
	MaxConcurrent uint32 `protobuf:"varint,3,opt,name=max_concurrent,json=maxConcurrent,proto3" json:"max_concurrent,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RateLimit) Reset() {
	*x = RateLimit{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RateLimit) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RateLimit) ProtoMessage() {}

func (x *RateLimit) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RateLimit.ProtoReflect.Descriptor instead.
func (*RateLimit) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{11}
}

func (x *RateLimit) GetRequestsPerMinute() uint32 {
	if x != nil {
		return x.RequestsPerMinute
	}
	return 0
}

func (x *RateLimit) GetTokensPerMinute() uint32 {
	if x != nil {
		return x.TokensPerMinute
	}
	return 0
}

func (x *RateLimit) GetMaxConcurrent() uint32 {
	if x != nil {
		return x.MaxConcurrent
	}
	return 0
}

type Principal struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	KeyId  string                 `protobuf:"bytes,1,opt,name=key_id,json=keyId,proto3" json:"key_id,omitempty"`
	Tenant string                 `protobuf:"bytes,2,opt,name=tenant,proto3" json:"tenant,omitempty"`
	Org    string                 `protobuf:"bytes,3,opt,name=org,proto3" json:"org,omitempty"`
	Team   string                 `protobuf:"bytes,4,opt,name=team,proto3" json:"team,omitempty"`
	User   string                 `protobuf:"bytes,5,opt,name=user,proto3" json:"user,omitempty"`
	App    string                 `protobuf:"bytes,6,opt,name=app,proto3" json:"app,omitempty"`
	Roles  []string               `protobuf:"bytes,7,rep,name=roles,proto3" json:"roles,omitempty"`
	// allow_all is explicit rather than implied by an empty list, so that an
	// empty allowlist means "nothing" and a builder that forgets to populate it
	// fails closed.
	ModelsAllowAll   bool         `protobuf:"varint,8,opt,name=models_allow_all,json=modelsAllowAll,proto3" json:"models_allow_all,omitempty"`
	Models           []string     `protobuf:"bytes,9,rep,name=models,proto3" json:"models,omitempty"`
	Budgets          []*BudgetRef `protobuf:"bytes,10,rep,name=budgets,proto3" json:"budgets,omitempty"`
	DefaultDataClass string       `protobuf:"bytes,11,opt,name=default_data_class,json=defaultDataClass,proto3" json:"default_data_class,omitempty"`
	MinTrustTier     TrustTier    `protobuf:"varint,12,opt,name=min_trust_tier,json=minTrustTier,proto3,enum=gateway.v1.TrustTier" json:"min_trust_tier,omitempty"`
	MaxConcurrent    uint32       `protobuf:"varint,13,opt,name=max_concurrent,json=maxConcurrent,proto3" json:"max_concurrent,omitempty"`
	// The outgoing generation of a rotated key stays valid until not_after, with
	// a warning header, so rotation is a non-event for callers.
	Deprecated     bool  `protobuf:"varint,14,opt,name=deprecated,proto3" json:"deprecated,omitempty"`
	NotAfterUnixMs int64 `protobuf:"varint,15,opt,name=not_after_unix_ms,json=notAfterUnixMs,proto3" json:"not_after_unix_ms,omitempty"`
	// Field 16: max_concurrent above is retained at 13 for compatibility and is
	// superseded by limits.max_concurrent. A field tag is never reused.
	Limits        *RateLimit `protobuf:"bytes,16,opt,name=limits,proto3" json:"limits,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Principal) Reset() {
	*x = Principal{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Principal) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Principal) ProtoMessage() {}

func (x *Principal) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Principal.ProtoReflect.Descriptor instead.
func (*Principal) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{12}
}

func (x *Principal) GetKeyId() string {
	if x != nil {
		return x.KeyId
	}
	return ""
}

func (x *Principal) GetTenant() string {
	if x != nil {
		return x.Tenant
	}
	return ""
}

func (x *Principal) GetOrg() string {
	if x != nil {
		return x.Org
	}
	return ""
}

func (x *Principal) GetTeam() string {
	if x != nil {
		return x.Team
	}
	return ""
}

func (x *Principal) GetUser() string {
	if x != nil {
		return x.User
	}
	return ""
}

func (x *Principal) GetApp() string {
	if x != nil {
		return x.App
	}
	return ""
}

func (x *Principal) GetRoles() []string {
	if x != nil {
		return x.Roles
	}
	return nil
}

func (x *Principal) GetModelsAllowAll() bool {
	if x != nil {
		return x.ModelsAllowAll
	}
	return false
}

func (x *Principal) GetModels() []string {
	if x != nil {
		return x.Models
	}
	return nil
}

func (x *Principal) GetBudgets() []*BudgetRef {
	if x != nil {
		return x.Budgets
	}
	return nil
}

func (x *Principal) GetDefaultDataClass() string {
	if x != nil {
		return x.DefaultDataClass
	}
	return ""
}

func (x *Principal) GetMinTrustTier() TrustTier {
	if x != nil {
		return x.MinTrustTier
	}
	return TrustTier_TRUST_TIER_UNSPECIFIED
}

func (x *Principal) GetMaxConcurrent() uint32 {
	if x != nil {
		return x.MaxConcurrent
	}
	return 0
}

func (x *Principal) GetDeprecated() bool {
	if x != nil {
		return x.Deprecated
	}
	return false
}

func (x *Principal) GetNotAfterUnixMs() int64 {
	if x != nil {
		return x.NotAfterUnixMs
	}
	return 0
}

func (x *Principal) GetLimits() *RateLimit {
	if x != nil {
		return x.Limits
	}
	return nil
}

// KeyEntry maps a key's lookup value to the principal it resolves to. The
// lookup is HMAC-SHA256 of the key secret under a pepper the worker holds and
// the snapshot never carries, so a stolen snapshot yields no usable key.
type KeyEntry struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Lookup        []byte                 `protobuf:"bytes,1,opt,name=lookup,proto3" json:"lookup,omitempty"`
	KeyId         string                 `protobuf:"bytes,2,opt,name=key_id,json=keyId,proto3" json:"key_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *KeyEntry) Reset() {
	*x = KeyEntry{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *KeyEntry) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*KeyEntry) ProtoMessage() {}

func (x *KeyEntry) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use KeyEntry.ProtoReflect.Descriptor instead.
func (*KeyEntry) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{13}
}

func (x *KeyEntry) GetLookup() []byte {
	if x != nil {
		return x.Lookup
	}
	return nil
}

func (x *KeyEntry) GetKeyId() string {
	if x != nil {
		return x.KeyId
	}
	return ""
}

// GlobalLayer is the tenant-independent half of a snapshot: large, and changes
// rarely.
type GlobalLayer struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Version       *LayerVersion          `protobuf:"bytes,1,opt,name=version,proto3" json:"version,omitempty"`
	BuiltAtUnixMs int64                  `protobuf:"varint,2,opt,name=built_at_unix_ms,json=builtAtUnixMs,proto3" json:"built_at_unix_ms,omitempty"`
	Deployments   []*Deployment          `protobuf:"bytes,3,rep,name=deployments,proto3" json:"deployments,omitempty"`
	Aliases       []*ModelAlias          `protobuf:"bytes,4,rep,name=aliases,proto3" json:"aliases,omitempty"`
	// Routes an API key's prefix segment to a tenant, so principal lookup is one
	// probe into one tenant's map rather than a scan across every tenant.
	TenantPrefixes  map[string]string `protobuf:"bytes,5,rep,name=tenant_prefixes,json=tenantPrefixes,proto3" json:"tenant_prefixes,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	DefaultPlugins  []*PluginBinding  `protobuf:"bytes,6,rep,name=default_plugins,json=defaultPlugins,proto3" json:"default_plugins,omitempty"`
	PolicyBundleRef string            `protobuf:"bytes,7,opt,name=policy_bundle_ref,json=policyBundleRef,proto3" json:"policy_bundle_ref,omitempty"`
	// Guardrails applied to any tenant that declares none of its own.
	DefaultGuardrails []*GuardrailBinding `protobuf:"bytes,8,rep,name=default_guardrails,json=defaultGuardrails,proto3" json:"default_guardrails,omitempty"`
	// The fleet-wide compiled policy, applied to tenants with none of their own.
	DefaultPolicy *PolicyBundle `protobuf:"bytes,9,opt,name=default_policy,json=defaultPolicy,proto3" json:"default_policy,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GlobalLayer) Reset() {
	*x = GlobalLayer{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GlobalLayer) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GlobalLayer) ProtoMessage() {}

func (x *GlobalLayer) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GlobalLayer.ProtoReflect.Descriptor instead.
func (*GlobalLayer) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{14}
}

func (x *GlobalLayer) GetVersion() *LayerVersion {
	if x != nil {
		return x.Version
	}
	return nil
}

func (x *GlobalLayer) GetBuiltAtUnixMs() int64 {
	if x != nil {
		return x.BuiltAtUnixMs
	}
	return 0
}

func (x *GlobalLayer) GetDeployments() []*Deployment {
	if x != nil {
		return x.Deployments
	}
	return nil
}

func (x *GlobalLayer) GetAliases() []*ModelAlias {
	if x != nil {
		return x.Aliases
	}
	return nil
}

func (x *GlobalLayer) GetTenantPrefixes() map[string]string {
	if x != nil {
		return x.TenantPrefixes
	}
	return nil
}

func (x *GlobalLayer) GetDefaultPlugins() []*PluginBinding {
	if x != nil {
		return x.DefaultPlugins
	}
	return nil
}

func (x *GlobalLayer) GetPolicyBundleRef() string {
	if x != nil {
		return x.PolicyBundleRef
	}
	return ""
}

func (x *GlobalLayer) GetDefaultGuardrails() []*GuardrailBinding {
	if x != nil {
		return x.DefaultGuardrails
	}
	return nil
}

func (x *GlobalLayer) GetDefaultPolicy() *PolicyBundle {
	if x != nil {
		return x.DefaultPolicy
	}
	return nil
}

// TenantLayer is one tenant's slice: small, and changes constantly. It is
// versioned and shipped independently of the global layer, so a budget edit
// does not reship the catalog to every worker.
type TenantLayer struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Tenant        string                 `protobuf:"bytes,1,opt,name=tenant,proto3" json:"tenant,omitempty"`
	Version       *LayerVersion          `protobuf:"bytes,2,opt,name=version,proto3" json:"version,omitempty"`
	BuiltAtUnixMs int64                  `protobuf:"varint,3,opt,name=built_at_unix_ms,json=builtAtUnixMs,proto3" json:"built_at_unix_ms,omitempty"`
	// Plan tier. Safe as a metrics label, unlike the tenant id, which is
	// unbounded cardinality and will take Prometheus down.
	Tier           string           `protobuf:"bytes,4,opt,name=tier,proto3" json:"tier,omitempty"`
	Principals     []*Principal     `protobuf:"bytes,5,rep,name=principals,proto3" json:"principals,omitempty"`
	Keys           []*KeyEntry      `protobuf:"bytes,6,rep,name=keys,proto3" json:"keys,omitempty"`
	AliasOverrides []*ModelAlias    `protobuf:"bytes,7,rep,name=alias_overrides,json=aliasOverrides,proto3" json:"alias_overrides,omitempty"`
	Budgets        []*BudgetState   `protobuf:"bytes,8,rep,name=budgets,proto3" json:"budgets,omitempty"`
	Plugins        []*PluginBinding `protobuf:"bytes,9,rep,name=plugins,proto3" json:"plugins,omitempty"`
	MinTrustTier   TrustTier        `protobuf:"varint,10,opt,name=min_trust_tier,json=minTrustTier,proto3,enum=gateway.v1.TrustTier" json:"min_trust_tier,omitempty"`
	// Guardrails for this tenant. A tenant that declares any replaces the fleet
	// defaults entirely rather than merging with them: merging two lists of
	// things that can refuse traffic produces a set nobody can predict, and
	// "which guardrails am I actually running" must have a simple answer.
	Guardrails []*GuardrailBinding `protobuf:"bytes,11,rep,name=guardrails,proto3" json:"guardrails,omitempty"`
	// This tenant's compiled policy. Replaces the fleet default rather than
	// merging, for the same reason guardrails do: two ordered rule lists merged
	// produce an order nobody can predict.
	Policy        *PolicyBundle `protobuf:"bytes,12,opt,name=policy,proto3" json:"policy,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TenantLayer) Reset() {
	*x = TenantLayer{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TenantLayer) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TenantLayer) ProtoMessage() {}

func (x *TenantLayer) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TenantLayer.ProtoReflect.Descriptor instead.
func (*TenantLayer) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{15}
}

func (x *TenantLayer) GetTenant() string {
	if x != nil {
		return x.Tenant
	}
	return ""
}

func (x *TenantLayer) GetVersion() *LayerVersion {
	if x != nil {
		return x.Version
	}
	return nil
}

func (x *TenantLayer) GetBuiltAtUnixMs() int64 {
	if x != nil {
		return x.BuiltAtUnixMs
	}
	return 0
}

func (x *TenantLayer) GetTier() string {
	if x != nil {
		return x.Tier
	}
	return ""
}

func (x *TenantLayer) GetPrincipals() []*Principal {
	if x != nil {
		return x.Principals
	}
	return nil
}

func (x *TenantLayer) GetKeys() []*KeyEntry {
	if x != nil {
		return x.Keys
	}
	return nil
}

func (x *TenantLayer) GetAliasOverrides() []*ModelAlias {
	if x != nil {
		return x.AliasOverrides
	}
	return nil
}

func (x *TenantLayer) GetBudgets() []*BudgetState {
	if x != nil {
		return x.Budgets
	}
	return nil
}

func (x *TenantLayer) GetPlugins() []*PluginBinding {
	if x != nil {
		return x.Plugins
	}
	return nil
}

func (x *TenantLayer) GetMinTrustTier() TrustTier {
	if x != nil {
		return x.MinTrustTier
	}
	return TrustTier_TRUST_TIER_UNSPECIFIED
}

func (x *TenantLayer) GetGuardrails() []*GuardrailBinding {
	if x != nil {
		return x.Guardrails
	}
	return nil
}

func (x *TenantLayer) GetPolicy() *PolicyBundle {
	if x != nil {
		return x.Policy
	}
	return nil
}

// Snapshot is a global layer plus the tenant layers composed onto it. Layers
// are also transferable on their own; this message is the full-fetch form used
// on worker startup and after a watch stream gap.
type Snapshot struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Named global_layer rather than global: the field name is not on the wire —
	// tag numbers are — but `global` is a reserved word in Python, which makes
	// the generated binding reachable only through getattr(). A name that is
	// awkward in one of the two languages that must read this is the wrong name.
	GlobalLayer   *GlobalLayer   `protobuf:"bytes,1,opt,name=global_layer,json=globalLayer,proto3" json:"global_layer,omitempty"`
	Tenants       []*TenantLayer `protobuf:"bytes,2,rep,name=tenants,proto3" json:"tenants,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Snapshot) Reset() {
	*x = Snapshot{}
	mi := &file_gateway_v1_snapshot_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Snapshot) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Snapshot) ProtoMessage() {}

func (x *Snapshot) ProtoReflect() protoreflect.Message {
	mi := &file_gateway_v1_snapshot_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Snapshot.ProtoReflect.Descriptor instead.
func (*Snapshot) Descriptor() ([]byte, []int) {
	return file_gateway_v1_snapshot_proto_rawDescGZIP(), []int{16}
}

func (x *Snapshot) GetGlobalLayer() *GlobalLayer {
	if x != nil {
		return x.GlobalLayer
	}
	return nil
}

func (x *Snapshot) GetTenants() []*TenantLayer {
	if x != nil {
		return x.Tenants
	}
	return nil
}

var File_gateway_v1_snapshot_proto protoreflect.FileDescriptor

const file_gateway_v1_snapshot_proto_rawDesc = "" +
	"\n" +
	"\x19gateway/v1/snapshot.proto\x12\n" +
	"gateway.v1\">\n" +
	"\fLayerVersion\x12\x16\n" +
	"\x06number\x18\x01 \x01(\x04R\x06number\x12\x16\n" +
	"\x06digest\x18\x02 \x01(\tR\x06digest\"J\n" +
	"\n" +
	"RoutingKey\x12\x1d\n" +
	"\n" +
	"base_model\x18\x01 \x01(\tR\tbaseModel\x12\x1d\n" +
	"\n" +
	"adapter_id\x18\x02 \x01(\tR\tadapterId\"\xf0\x01\n" +
	"\x04Cost\x122\n" +
	"\x16input_per_1k_micro_usd\x18\x01 \x01(\x03R\x12inputPer1kMicroUsd\x124\n" +
	"\x17output_per_1k_micro_usd\x18\x02 \x01(\x03R\x13outputPer1kMicroUsd\x12?\n" +
	"\x1dcached_input_per_1k_micro_usd\x18\x03 \x01(\x03R\x18cachedInputPer1kMicroUsd\x12=\n" +
	"\x1ccache_write_per_1k_micro_usd\x18\x04 \x01(\x03R\x17cacheWritePer1kMicroUsd\"\xd5\x02\n" +
	"\n" +
	"Deployment\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12(\n" +
	"\x03key\x18\x02 \x01(\v2\x16.gateway.v1.RoutingKeyR\x03key\x12\x1a\n" +
	"\bprovider\x18\x03 \x01(\tR\bprovider\x12\x1a\n" +
	"\bendpoint\x18\x04 \x01(\tR\bendpoint\x12\x16\n" +
	"\x06region\x18\x05 \x01(\tR\x06region\x124\n" +
	"\n" +
	"trust_tier\x18\x06 \x01(\x0e2\x15.gateway.v1.TrustTierR\ttrustTier\x12%\n" +
	"\x0ecredential_ref\x18\a \x01(\tR\rcredentialRef\x12\x16\n" +
	"\x06weight\x18\b \x01(\rR\x06weight\x12$\n" +
	"\x04cost\x18\t \x01(\v2\x10.gateway.v1.CostR\x04cost\x12\"\n" +
	"\fcapabilities\x18\n" +
	" \x03(\tR\fcapabilities\"R\n" +
	"\n" +
	"ModelAlias\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x120\n" +
	"\atargets\x18\x02 \x03(\v2\x16.gateway.v1.RoutingKeyR\atargets\"\xf7\x02\n" +
	"\n" +
	"PolicyRule\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x120\n" +
	"\x06effect\x18\x02 \x01(\x0e2\x18.gateway.v1.PolicyEffectR\x06effect\x12\x16\n" +
	"\x06models\x18\x03 \x03(\tR\x06models\x12\x1c\n" +
	"\tendpoints\x18\x04 \x03(\tR\tendpoints\x12\x14\n" +
	"\x05roles\x18\x05 \x03(\tR\x05roles\x12\x18\n" +
	"\aregions\x18\x06 \x03(\tR\aregions\x12!\n" +
	"\fsource_cidrs\x18\a \x03(\tR\vsourceCidrs\x12*\n" +
	"\x11max_payload_bytes\x18\b \x01(\x04R\x0fmaxPayloadBytes\x12\x1d\n" +
	"\n" +
	"data_class\x18\t \x01(\tR\tdataClass\x12;\n" +
	"\x0emin_trust_tier\x18\n" +
	" \x01(\x0e2\x15.gateway.v1.TrustTierR\fminTrustTier\x12\x16\n" +
	"\x06reason\x18\v \x01(\tR\x06reason\"\xa7\x01\n" +
	"\fPolicyBundle\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x04R\aversion\x12,\n" +
	"\x05rules\x18\x03 \x03(\v2\x16.gateway.v1.PolicyRuleR\x05rules\x12?\n" +
	"\x0edefault_effect\x18\x04 \x01(\x0e2\x18.gateway.v1.PolicyEffectR\rdefaultEffect\"\xf8\x01\n" +
	"\x10GuardrailBinding\x12\x1c\n" +
	"\tcomponent\x18\x01 \x01(\tR\tcomponent\x12\x18\n" +
	"\aversion\x18\x02 \x01(\tR\aversion\x12\x1d\n" +
	"\n" +
	"config_ref\x18\x03 \x01(\tR\tconfigRef\x12\x1d\n" +
	"\n" +
	"timeout_ms\x18\x04 \x01(\rR\ttimeoutMs\x12:\n" +
	"\ffailure_mode\x18\x05 \x01(\x0e2\x17.gateway.v1.FailureModeR\vfailureMode\x12\x1a\n" +
	"\bblocking\x18\x06 \x01(\bR\bblocking\x12\x16\n" +
	"\x06phases\x18\a \x03(\tR\x06phases\"\x8c\x01\n" +
	"\rPluginBinding\x12$\n" +
	"\x04port\x18\x01 \x01(\x0e2\x10.gateway.v1.PortR\x04port\x12\x1c\n" +
	"\tcomponent\x18\x02 \x01(\tR\tcomponent\x12\x18\n" +
	"\aversion\x18\x03 \x01(\tR\aversion\x12\x1d\n" +
	"\n" +
	"config_ref\x18\x04 \x01(\tR\tconfigRef\"\xe4\x01\n" +
	"\vBudgetState\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12-\n" +
	"\x05scope\x18\x02 \x01(\x0e2\x17.gateway.v1.BudgetScopeR\x05scope\x12&\n" +
	"\x0flimit_micro_usd\x18\x03 \x01(\x03R\rlimitMicroUsd\x12&\n" +
	"\x0fspent_micro_usd\x18\x04 \x01(\x03R\rspentMicroUsd\x12\x12\n" +
	"\x04hard\x18\x05 \x01(\bR\x04hard\x122\n" +
	"\x15headroom_basis_points\x18\x06 \x01(\rR\x13headroomBasisPoints\"J\n" +
	"\tBudgetRef\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12-\n" +
	"\x05scope\x18\x02 \x01(\x0e2\x17.gateway.v1.BudgetScopeR\x05scope\"\x8e\x01\n" +
	"\tRateLimit\x12.\n" +
	"\x13requests_per_minute\x18\x01 \x01(\rR\x11requestsPerMinute\x12*\n" +
	"\x11tokens_per_minute\x18\x02 \x01(\rR\x0ftokensPerMinute\x12%\n" +
	"\x0emax_concurrent\x18\x03 \x01(\rR\rmaxConcurrent\"\x9b\x04\n" +
	"\tPrincipal\x12\x15\n" +
	"\x06key_id\x18\x01 \x01(\tR\x05keyId\x12\x16\n" +
	"\x06tenant\x18\x02 \x01(\tR\x06tenant\x12\x10\n" +
	"\x03org\x18\x03 \x01(\tR\x03org\x12\x12\n" +
	"\x04team\x18\x04 \x01(\tR\x04team\x12\x12\n" +
	"\x04user\x18\x05 \x01(\tR\x04user\x12\x10\n" +
	"\x03app\x18\x06 \x01(\tR\x03app\x12\x14\n" +
	"\x05roles\x18\a \x03(\tR\x05roles\x12(\n" +
	"\x10models_allow_all\x18\b \x01(\bR\x0emodelsAllowAll\x12\x16\n" +
	"\x06models\x18\t \x03(\tR\x06models\x12/\n" +
	"\abudgets\x18\n" +
	" \x03(\v2\x15.gateway.v1.BudgetRefR\abudgets\x12,\n" +
	"\x12default_data_class\x18\v \x01(\tR\x10defaultDataClass\x12;\n" +
	"\x0emin_trust_tier\x18\f \x01(\x0e2\x15.gateway.v1.TrustTierR\fminTrustTier\x12%\n" +
	"\x0emax_concurrent\x18\r \x01(\rR\rmaxConcurrent\x12\x1e\n" +
	"\n" +
	"deprecated\x18\x0e \x01(\bR\n" +
	"deprecated\x12)\n" +
	"\x11not_after_unix_ms\x18\x0f \x01(\x03R\x0enotAfterUnixMs\x12-\n" +
	"\x06limits\x18\x10 \x01(\v2\x15.gateway.v1.RateLimitR\x06limits\"9\n" +
	"\bKeyEntry\x12\x16\n" +
	"\x06lookup\x18\x01 \x01(\fR\x06lookup\x12\x15\n" +
	"\x06key_id\x18\x02 \x01(\tR\x05keyId\"\xed\x04\n" +
	"\vGlobalLayer\x122\n" +
	"\aversion\x18\x01 \x01(\v2\x18.gateway.v1.LayerVersionR\aversion\x12'\n" +
	"\x10built_at_unix_ms\x18\x02 \x01(\x03R\rbuiltAtUnixMs\x128\n" +
	"\vdeployments\x18\x03 \x03(\v2\x16.gateway.v1.DeploymentR\vdeployments\x120\n" +
	"\aaliases\x18\x04 \x03(\v2\x16.gateway.v1.ModelAliasR\aaliases\x12T\n" +
	"\x0ftenant_prefixes\x18\x05 \x03(\v2+.gateway.v1.GlobalLayer.TenantPrefixesEntryR\x0etenantPrefixes\x12B\n" +
	"\x0fdefault_plugins\x18\x06 \x03(\v2\x19.gateway.v1.PluginBindingR\x0edefaultPlugins\x12*\n" +
	"\x11policy_bundle_ref\x18\a \x01(\tR\x0fpolicyBundleRef\x12K\n" +
	"\x12default_guardrails\x18\b \x03(\v2\x1c.gateway.v1.GuardrailBindingR\x11defaultGuardrails\x12?\n" +
	"\x0edefault_policy\x18\t \x01(\v2\x18.gateway.v1.PolicyBundleR\rdefaultPolicy\x1aA\n" +
	"\x13TenantPrefixesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"\xcd\x04\n" +
	"\vTenantLayer\x12\x16\n" +
	"\x06tenant\x18\x01 \x01(\tR\x06tenant\x122\n" +
	"\aversion\x18\x02 \x01(\v2\x18.gateway.v1.LayerVersionR\aversion\x12'\n" +
	"\x10built_at_unix_ms\x18\x03 \x01(\x03R\rbuiltAtUnixMs\x12\x12\n" +
	"\x04tier\x18\x04 \x01(\tR\x04tier\x125\n" +
	"\n" +
	"principals\x18\x05 \x03(\v2\x15.gateway.v1.PrincipalR\n" +
	"principals\x12(\n" +
	"\x04keys\x18\x06 \x03(\v2\x14.gateway.v1.KeyEntryR\x04keys\x12?\n" +
	"\x0falias_overrides\x18\a \x03(\v2\x16.gateway.v1.ModelAliasR\x0ealiasOverrides\x121\n" +
	"\abudgets\x18\b \x03(\v2\x17.gateway.v1.BudgetStateR\abudgets\x123\n" +
	"\aplugins\x18\t \x03(\v2\x19.gateway.v1.PluginBindingR\aplugins\x12;\n" +
	"\x0emin_trust_tier\x18\n" +
	" \x01(\x0e2\x15.gateway.v1.TrustTierR\fminTrustTier\x12<\n" +
	"\n" +
	"guardrails\x18\v \x03(\v2\x1c.gateway.v1.GuardrailBindingR\n" +
	"guardrails\x120\n" +
	"\x06policy\x18\f \x01(\v2\x18.gateway.v1.PolicyBundleR\x06policy\"y\n" +
	"\bSnapshot\x12:\n" +
	"\fglobal_layer\x18\x01 \x01(\v2\x17.gateway.v1.GlobalLayerR\vglobalLayer\x121\n" +
	"\atenants\x18\x02 \x03(\v2\x17.gateway.v1.TenantLayerR\atenants*w\n" +
	"\tTrustTier\x12\x1a\n" +
	"\x16TRUST_TIER_UNSPECIFIED\x10\x00\x12\x17\n" +
	"\x13TRUST_TIER_EXTERNAL\x10\x01\x12\x1c\n" +
	"\x18TRUST_TIER_PRIVATE_CLOUD\x10\x02\x12\x17\n" +
	"\x13TRUST_TIER_INTERNAL\x10\x03*\xce\x01\n" +
	"\vBudgetScope\x12\x1c\n" +
	"\x18BUDGET_SCOPE_UNSPECIFIED\x10\x00\x12\x14\n" +
	"\x10BUDGET_SCOPE_KEY\x10\x01\x12\x14\n" +
	"\x10BUDGET_SCOPE_APP\x10\x02\x12\x15\n" +
	"\x11BUDGET_SCOPE_USER\x10\x03\x12\x15\n" +
	"\x11BUDGET_SCOPE_TEAM\x10\x04\x12\x14\n" +
	"\x10BUDGET_SCOPE_ORG\x10\x05\x12\x16\n" +
	"\x12BUDGET_SCOPE_MODEL\x10\x06\x12\x19\n" +
	"\x15BUDGET_SCOPE_TRAINING\x10\a*g\n" +
	"\x04Port\x12\x14\n" +
	"\x10PORT_UNSPECIFIED\x10\x00\x12\x11\n" +
	"\rPORT_PROVIDER\x10\x01\x12\x12\n" +
	"\x0ePORT_GUARDRAIL\x10\x02\x12\x0e\n" +
	"\n" +
	"PORT_STORE\x10\x03\x12\x12\n" +
	"\x0ePORT_TELEMETRY\x10\x04*[\n" +
	"\vFailureMode\x12\x1c\n" +
	"\x18FAILURE_MODE_UNSPECIFIED\x10\x00\x12\x15\n" +
	"\x11FAILURE_MODE_OPEN\x10\x01\x12\x17\n" +
	"\x13FAILURE_MODE_CLOSED\x10\x02*^\n" +
	"\fPolicyEffect\x12\x1d\n" +
	"\x19POLICY_EFFECT_UNSPECIFIED\x10\x00\x12\x17\n" +
	"\x13POLICY_EFFECT_ALLOW\x10\x01\x12\x16\n" +
	"\x12POLICY_EFFECT_DENY\x10\x02BIZGgithub.com/umerjavaidkh/model-gateway/dataplane/internal/wire/gatewayv1b\x06proto3"

var (
	file_gateway_v1_snapshot_proto_rawDescOnce sync.Once
	file_gateway_v1_snapshot_proto_rawDescData []byte
)

func file_gateway_v1_snapshot_proto_rawDescGZIP() []byte {
	file_gateway_v1_snapshot_proto_rawDescOnce.Do(func() {
		file_gateway_v1_snapshot_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_gateway_v1_snapshot_proto_rawDesc), len(file_gateway_v1_snapshot_proto_rawDesc)))
	})
	return file_gateway_v1_snapshot_proto_rawDescData
}

var file_gateway_v1_snapshot_proto_enumTypes = make([]protoimpl.EnumInfo, 5)
var file_gateway_v1_snapshot_proto_msgTypes = make([]protoimpl.MessageInfo, 18)
var file_gateway_v1_snapshot_proto_goTypes = []any{
	(TrustTier)(0),           // 0: gateway.v1.TrustTier
	(BudgetScope)(0),         // 1: gateway.v1.BudgetScope
	(Port)(0),                // 2: gateway.v1.Port
	(FailureMode)(0),         // 3: gateway.v1.FailureMode
	(PolicyEffect)(0),        // 4: gateway.v1.PolicyEffect
	(*LayerVersion)(nil),     // 5: gateway.v1.LayerVersion
	(*RoutingKey)(nil),       // 6: gateway.v1.RoutingKey
	(*Cost)(nil),             // 7: gateway.v1.Cost
	(*Deployment)(nil),       // 8: gateway.v1.Deployment
	(*ModelAlias)(nil),       // 9: gateway.v1.ModelAlias
	(*PolicyRule)(nil),       // 10: gateway.v1.PolicyRule
	(*PolicyBundle)(nil),     // 11: gateway.v1.PolicyBundle
	(*GuardrailBinding)(nil), // 12: gateway.v1.GuardrailBinding
	(*PluginBinding)(nil),    // 13: gateway.v1.PluginBinding
	(*BudgetState)(nil),      // 14: gateway.v1.BudgetState
	(*BudgetRef)(nil),        // 15: gateway.v1.BudgetRef
	(*RateLimit)(nil),        // 16: gateway.v1.RateLimit
	(*Principal)(nil),        // 17: gateway.v1.Principal
	(*KeyEntry)(nil),         // 18: gateway.v1.KeyEntry
	(*GlobalLayer)(nil),      // 19: gateway.v1.GlobalLayer
	(*TenantLayer)(nil),      // 20: gateway.v1.TenantLayer
	(*Snapshot)(nil),         // 21: gateway.v1.Snapshot
	nil,                      // 22: gateway.v1.GlobalLayer.TenantPrefixesEntry
}
var file_gateway_v1_snapshot_proto_depIdxs = []int32{
	6,  // 0: gateway.v1.Deployment.key:type_name -> gateway.v1.RoutingKey
	0,  // 1: gateway.v1.Deployment.trust_tier:type_name -> gateway.v1.TrustTier
	7,  // 2: gateway.v1.Deployment.cost:type_name -> gateway.v1.Cost
	6,  // 3: gateway.v1.ModelAlias.targets:type_name -> gateway.v1.RoutingKey
	4,  // 4: gateway.v1.PolicyRule.effect:type_name -> gateway.v1.PolicyEffect
	0,  // 5: gateway.v1.PolicyRule.min_trust_tier:type_name -> gateway.v1.TrustTier
	10, // 6: gateway.v1.PolicyBundle.rules:type_name -> gateway.v1.PolicyRule
	4,  // 7: gateway.v1.PolicyBundle.default_effect:type_name -> gateway.v1.PolicyEffect
	3,  // 8: gateway.v1.GuardrailBinding.failure_mode:type_name -> gateway.v1.FailureMode
	2,  // 9: gateway.v1.PluginBinding.port:type_name -> gateway.v1.Port
	1,  // 10: gateway.v1.BudgetState.scope:type_name -> gateway.v1.BudgetScope
	1,  // 11: gateway.v1.BudgetRef.scope:type_name -> gateway.v1.BudgetScope
	15, // 12: gateway.v1.Principal.budgets:type_name -> gateway.v1.BudgetRef
	0,  // 13: gateway.v1.Principal.min_trust_tier:type_name -> gateway.v1.TrustTier
	16, // 14: gateway.v1.Principal.limits:type_name -> gateway.v1.RateLimit
	5,  // 15: gateway.v1.GlobalLayer.version:type_name -> gateway.v1.LayerVersion
	8,  // 16: gateway.v1.GlobalLayer.deployments:type_name -> gateway.v1.Deployment
	9,  // 17: gateway.v1.GlobalLayer.aliases:type_name -> gateway.v1.ModelAlias
	22, // 18: gateway.v1.GlobalLayer.tenant_prefixes:type_name -> gateway.v1.GlobalLayer.TenantPrefixesEntry
	13, // 19: gateway.v1.GlobalLayer.default_plugins:type_name -> gateway.v1.PluginBinding
	12, // 20: gateway.v1.GlobalLayer.default_guardrails:type_name -> gateway.v1.GuardrailBinding
	11, // 21: gateway.v1.GlobalLayer.default_policy:type_name -> gateway.v1.PolicyBundle
	5,  // 22: gateway.v1.TenantLayer.version:type_name -> gateway.v1.LayerVersion
	17, // 23: gateway.v1.TenantLayer.principals:type_name -> gateway.v1.Principal
	18, // 24: gateway.v1.TenantLayer.keys:type_name -> gateway.v1.KeyEntry
	9,  // 25: gateway.v1.TenantLayer.alias_overrides:type_name -> gateway.v1.ModelAlias
	14, // 26: gateway.v1.TenantLayer.budgets:type_name -> gateway.v1.BudgetState
	13, // 27: gateway.v1.TenantLayer.plugins:type_name -> gateway.v1.PluginBinding
	0,  // 28: gateway.v1.TenantLayer.min_trust_tier:type_name -> gateway.v1.TrustTier
	12, // 29: gateway.v1.TenantLayer.guardrails:type_name -> gateway.v1.GuardrailBinding
	11, // 30: gateway.v1.TenantLayer.policy:type_name -> gateway.v1.PolicyBundle
	19, // 31: gateway.v1.Snapshot.global_layer:type_name -> gateway.v1.GlobalLayer
	20, // 32: gateway.v1.Snapshot.tenants:type_name -> gateway.v1.TenantLayer
	33, // [33:33] is the sub-list for method output_type
	33, // [33:33] is the sub-list for method input_type
	33, // [33:33] is the sub-list for extension type_name
	33, // [33:33] is the sub-list for extension extendee
	0,  // [0:33] is the sub-list for field type_name
}

func init() { file_gateway_v1_snapshot_proto_init() }
func file_gateway_v1_snapshot_proto_init() {
	if File_gateway_v1_snapshot_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_gateway_v1_snapshot_proto_rawDesc), len(file_gateway_v1_snapshot_proto_rawDesc)),
			NumEnums:      5,
			NumMessages:   18,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_gateway_v1_snapshot_proto_goTypes,
		DependencyIndexes: file_gateway_v1_snapshot_proto_depIdxs,
		EnumInfos:         file_gateway_v1_snapshot_proto_enumTypes,
		MessageInfos:      file_gateway_v1_snapshot_proto_msgTypes,
	}.Build()
	File_gateway_v1_snapshot_proto = out.File
	file_gateway_v1_snapshot_proto_goTypes = nil
	file_gateway_v1_snapshot_proto_depIdxs = nil
}
